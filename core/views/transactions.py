...
def transaction_list(request):
    ...
    # Split filter (requires the split_count annotation)
    if split_filter == 'yes':
...
def bulk_update_category(request):
    """Bulk assign a category to multiple transactions.

    Supports two modes:
    - Individual IDs via txn_ids (checkbox selection)
    - All matching via select_all_matching=1 (re-applies current filters)
    """
